...
namespace EP94.AsyncWorker.Internal.Models
{
    public class WorkFactory(int maxLevelOfConcurrency, TaskScheduler taskScheduler, TimeSpan? defaultTimeout, CancellationToken cancellationToken) : IWorkFactory
    {
        ...
        public IWorkHandle<T> CreateTimebasedTrigger<T>(FuncWorkDelegate<T> work, DateTime firstRun, Func<DateTime> getNextTimeFunc, string? name = null, CancellationToken cancellationToken = default) => new TimeBasedTrigger<T>(WorkDelegate.Create(work), firstRun, getNextTimeFunc, _workHandler, this, name, cancellationToken); 
