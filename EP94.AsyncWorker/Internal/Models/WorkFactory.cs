...
namespace EP94.AsyncWorker.Internal.Models
{
    public class WorkFactory(int maxLevelOfConcurrency, TaskScheduler taskScheduler, TimeSpan? defaultTimeout, CancellationToken cancellationToken) : IWorkFactory
    {
        ...
        public IFuncWorkHandle<T> CreateTimebasedTrigger<T>(FuncWorkDelegate<T> work, DateTime firstRun, Func<DateTime> getNextTimeFunc, string? name = null, CancellationToken cancellationToken = default) => new TimeBasedTrigger<T>(WorkDelegate.Create(work), firstRun, getNextTimeFunc, _workHandler, this, name, cancellationToken); 
        public ITrigger<TParam> CreateTriggerAsync<TParam>(bool triggerOnlyOnChanges = false, string? name = null, IEqualityComparer<TParam>? equalityComparer = null) => new Trigger<TParam>(_workHandler, this, name, triggerOnlyOnChanges, equalityComparer ?? EqualityComparer<TParam>.Default, cancellationToken);
        public ITrigger<TParam> CreateTriggerAsync<TParam>(TParam? initialValue, bool triggerOnlyOnChanges = false, string? name = null, IEqualityComparer<TParam>? equalityComparer = null) => new Trigger<TParam>(_workHandler, this, initialValue, name, triggerOnlyOnChanges, equalityComparer ?? EqualityComparer<TParam>.Default, cancellationToken);
